        ...
        <>
            {props.navbar &&
                <div className="hidden md:block relative select-none" ref={cartRef}>
                    ...
                    >
                        <FontAwesomeIcon icon={faShoppingCart} className="max-h-4" /> {cart.length > 0 &&
                            <span className="absolute -top-2 -left-2 p-0.5 bg-emerald-500/75 group-hover:bg-emerald-500 text-white text-xs font-bold rounded-full">{cart.length}</span>
                        }
                        Cart
            ...
            {props.sidebar &&
                <div className="block md:hidden relative select-none" ref={cartRef}>
                    <div
                        ...
                        onClick={() => setCartOpen(!cartOpen)}
                    >
                        <FontAwesomeIcon icon={faShoppingCart} /> {cart.length > 0 &&
                            <span className="absolute -top-2 -left-2 p-0.5 bg-emerald-500/75 group-hover:bg-emerald-500 text-white text-xs font-bold rounded-full">{cart.length}</span>
                        }
